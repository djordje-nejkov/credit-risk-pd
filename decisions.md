...
Test 4 is set relative, so it only applies to Set 2, where features derived from Lending Club's own model are excluded. Installment is a feature that will be included in Set 1, but not in Set 2, since in combination with loan_amnt it can recover int_rate. Further on this in #5. 
...
Lastly, the 'since' columns (see explore_features.py) will get filled with median values learned on the training slices per #6, as well as each one having an additional indicator column indicating whether the value was originally NaN. Using only the median would misrepresent the borrowers with no delinquency, public records or recent inquiries. For example, mths_since_last_record's blank group which has 0.1424 pd would get merged with the present group, which has 0.1765 pd.
...
Regarding LR's filled rows, the rejected alternatives were: 1. to drop the NaN rows, which resulted in 3.07% of the cohort being kept, see explore_features.py and 2. using the median without an indicator. The fill values sit inside the observed range of the column's values. The median fills are fitted on the training slice as per decisions.md #6, and some features like bc_open_to_buy and bc_util are kept constant, rather than fitted, see explore_features.py as for why.

As for the shared bankcard indicator, 2,652/3,279 rows are missing all four columns, which is 81%. Having four separate indicators would be near-duplicates splitting one signal when we apply L2 penalisation. The rest that have 1-3/4 parameters missing get treated as missing all of them.
...
Decision: The threshold is on the 85th percentile of the score distribution, so the riskiest 15% of potential borrowers are declined. The measure is taken from Q3 and the matrix is reported on Q4. The same threshold applies to all four arms of the 2x2.

Why: The threshold is put in place since the approve/decline framing and the two errors are named in README.md, and the ranking metrics never make that decision. The matrix is the only place where the two errors could appear as counts.

A percentile of the applicants rather than a probability cut is used since the probabilities are not calibrated - see #10. A percentile is chosen only on the ordering, which is what the rest of the reported findings read. The 85th percentile is used as a cutoff, since LC's book is self-selected and riskier than a bank's, so a bank-tight cut is not the reference point. The measure is taken from Q3, since a cut chosen on the reported slice is the error the three-way split exists to prevent.

Cost: The 85th percentile is not a derived number, and a different one moves every cell. Nothing in the data would pick 15% over 20%, for example. Moreover, the matrix inherits the model's discrimination, not just the cut. Declining the riskiest 15% by score does not remove 15% of the bad loans, and the overlap is what AUC already reports.

Furthermore, the cut is a fixed score value taken from Q3, so what fraction of Q4 falls above it depends on where the two distributions sit, and it may be 12% or 20% rather than 15%.
